...
from suds.client import Client as Client_Prodigia
...
class HRPayslip(models.Model):
    ...
    def _get_cfdi_data_dict(self):
        ...
        receptor_rfc = self.employee_id.vat
        ...
        if not self.employee_id.address_id.state_id:
            error = _("Advertencia !!!\nNo ha definido el Estado en la dirección de la Empresa asociada al Trabajador, por favor configure ese dato y reintente la operación")
        ...
        if not self.contract_id.registro_patronal_id:
            error = _("Advertencia !!!\nNo ha definido el registro patronal para el Trabajador en el Contrato")    
        ...
        if self.company_id.version_de_cfdi_para_nominas=='3.3':
            ...
            if self.employee_id._fields.get('apellido_paterno', False):
                _logger.info("self.employee_id.name: %s" % self.employee_id.name)
        ...
        receptor_rfc = receptor_rfc and receptor_rfc.replace('&','&amp;') or ''
    ...
    def action_cancel(self):
        if not self.env.context.get('settlement_id', False) and any(x.settlement_id for x in self):
    ...
    def _l10n_mx_edi_prodigia_info(self, company_id, service_type):
        data =  {
            ...
            'url': 'http://timbrado.pade.mx:80/servicio/Timbrado3.3?wsdl',
            'cancel_url': 'https://timbrado.pade.mx/odoo/PadeOdooTimbradoService?wsdl',
        }
        if self.env.company.version_de_cfdi_para_nominas=='4.0':
            data.update({
                'url': 'http://timbrado.pade.mx:80/servicio/Timbrado4.0?wsdl',
    ...
    def _l10n_mx_edi_prodigia_sign(self, pac_info):
        ...
        for rec in self:
            try:
                _logger.info("rec.number: %s" % rec.number)
            ...
            msg = res['servicioTimbrado']['mensaje']
            code = res['servicioTimbrado']['codigo']
            _logger.info("msg: %s" % msg)
            _logger.info("code: %s" % code)
            if res['servicioTimbrado']['timbradoOk']!='false':
                xml_signed = res['servicioTimbrado']['xmlBase64']
                _logger.info("xml_signed: %s" % xml_signed)
                _logger.info("xml_signed: %s" % base64.b64decode(xml_signed.encode('utf-8')))
            else:
                xml_signed = b''
    ...
    def _l10n_mx_edi_prodigia_cancel(self, pac_info):
        ...
        for rec in self:
            ...
            rfc_receptor = rec.employee_id #partner_id
            if not rfc_receptor.vat:
                rec._l10n_mx_edi_post_cancel_process(False, '999', 'El Empleado no tiene RFC definido')
                continue
