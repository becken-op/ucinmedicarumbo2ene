...
class HRContractSueldoInverso(models.TransientModel):
    ...
    prestacion_premio_asistencia = fields.Float(string="Premio por Asistencia (%)", default=0, digits=(12, 2))
    prestacion_premio_puntualidad = fields.Float(string="Premio por Puntualidad  (%)", default=0, digits=(12, 2))
    prestacion_vales_despensa = fields.Float(string="Vales de Despensa (Monto Diario)", default=0, digits=(12, 2))


    monto_premio_asistencia  = fields.Float(string="Monto Premio por Puntualidad", default=0, digits=(12, 2))
    monto_premio_puntualidad = fields.Float(string="Monto Premio por Asistencia", default=0, digits=(12, 2))
    monto_vales_despensa = fields.Float(string="Monto Vales de Despensa", default=0, digits=(12, 2))

    monto_gravado = fields.Float(string="Monto Gravado", default=0, digits=(12, 2))
    monto_exento = fields.Float(string="Monto Exento", default=0, digits=(12, 2))
    monto_percepciones = fields.Float(string="Total Percepciones", default=0, digits=(12, 2))
    ...
    def calcular(self, monto_base):
        sueldo_diario = monto_base / dias[self.periodo]
        self.sueldo_bruto = monto_base
        self.sueldo_diario = sueldo_diario
        self.monto_premio_puntualidad = self.sueldo_bruto * self.prestacion_premio_puntualidad / 100.0
        self.monto_premio_asistencia = self.sueldo_bruto  * self.prestacion_premio_asistencia / 100.0
        self.monto_vales_despensa = dias[self.periodo] * self.prestacion_vales_despensa
        self.monto_gravado = self.sueldo_bruto + self.monto_premio_puntualidad + self.monto_premio_asistencia
        self.monto_exento = self.monto_vales_despensa
        self.monto_percepciones = self.sueldo_bruto + self.monto_premio_puntualidad + self.monto_premio_asistencia + self.monto_vales_despensa
        # Calcular el factor de integracion
        monto_aguinaldo = self.dias_aguinaldo * sueldo_diario
        monto_prima_vacacional = sueldo_diario * self.dias_vacaciones * self.porc_prima_vacacional / 100.0
        factor = ((sueldo_diario * 365.0) + monto_aguinaldo + monto_prima_vacacional) / (sueldo_diario * 365.0)
        self.sueldo_sbc = sueldo_diario * factor

        # Calculo del ISR
        gravada_promedio_mensual = (sueldo_diario + (sueldo_diario * self.prestacion_premio_puntualidad / 100.0) + (sueldo_diario * self.prestacion_premio_asistencia / 100.0)) * 30.4
        ...
        self.sueldo_neto = self.sueldo_bruto + self.monto_premio_asistencia + self.monto_premio_puntualidad + self.monto_vales_despensa - \
                            self.monto_isr + \
    ...
    def _onchange_prestacion_id(self):
        if self.prestacion_id:
            prest_line = self.prestacion_id.line_ids.filtered(lambda w: w.antiguedad==0 and w.sindicalizado==self.sindicalizado)
            if prest_line:
    ...
    @api.onchange('monto_base','tipo_sueldo', 'periodo', 'sindicalizado','prestacion_id', 'prestacion_premio_asistencia','prestacion_premio_puntualidad','prestacion_vales_despensa')
    def get_data(self):
        # Validaciones
