...
class HRPayrollPeriod(models.Model):
    ...
    date_account = fields.Selection([('date_to','Fecha Final'),
                                     ('date_payroll','Fecha de Pago')],
                                     string="Fecha Contable", default='date_payroll', required=True,
                                     help="Seleccione cuál fecha se tomará para generar la información contable")
    ...
    def compute_lines(self):
        ...
        journal_id = self.journal_id.id
        periodos = {'semanal'   : {'dias': 6, 'periodos' : 52},
                    'decenal'   : {'dias': 9, 'periodos' : 37},
                    'catorcenal': {'dias': 13,'periodos' : 26},
                    } 
        if self.period_type in ('semanal','decenal','catorcenal'):
            dias = periodos[self.period_type]['dias']
            ...
            while flag:
                ...
                cont += 1
                if cont > periodos[self.period_type]['periodos']:
                    break
                name = _('%s %s %s: %s') % (str(self.anio), self.code, str(cont), self.get_period_name(date_from, date_to))
                ...
                data.update({
                        ...
                        'date_account' : date_to if self.date_account=='date_to' else (date_to + timedelta(days=dias_para_pago)),
