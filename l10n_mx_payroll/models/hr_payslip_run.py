...
class HrPayslipRun(models.Model):
    ...
    def action_validate(self):
        self.slip_ids.filtered(lambda w: w.state in ('draft','verify')).action_payslip_done()
        self.action_close()
        return True
    ...
    def confirm_payslips(self):
        ...
        self.slip_ids.filtered(lambda w: w.state=='draft').action_payslip_done()
        return True
    """
    ################
    ...
    def action_cancel_draft(self):
        ...
        if self.to_cancel:
            slip_obj_cancel1=self.env['hr.payslip'].browse([])
            slip_obj_cancel2=self.env['hr.payslip'].browse([])
            for slip in self.slip_ids:
                if slip.state in ('draft','verify'):
                    slip_obj_cancel1 |= slip
                if slip.state == 'done':
                    slip_obj_cancel2 |= slip
            slip_obj_cancel1 and slip_obj_cancel1.action_payslip_cancel()
            slip_obj_cancel2 and slip_obj_cancel2.action_cancel()
    ...
    def action_view_analysis(self):
        return {
            ...
            'context'   : {'pivot_measures': ['amount'], 'pivot_row_groupby': ['employee_id'],'pivot_column_groupby': ['nomina_aplicacion','salary_rule_id'], 'search_default_nomina_aplicacion_percepciones':1, 'search_default_nomina_aplicacion_deducciones':1, 'search_default_nomina_aplicacion_otrospagos':1, 'search_default_no_suma_0':1},
            'view_mode' : 'pivot,graph',
            'domain'    : [('amount','!=',0), ('payslip_run_id','=',self.id)]
        }