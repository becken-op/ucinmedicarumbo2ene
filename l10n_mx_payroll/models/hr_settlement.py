...
class hr_settlement(models.Model):
    ...
    percepciones_with_otrospagos_ids = fields.One2many('hr.payslip.line', 'settlement_id', string='Percepciones + Otros Pagos', readonly=True,
    ...
    def _onchange_employee_id(self):
        if not self.employee_id:
            ...
            if contract:
                ...
                dias_aguinaldo = 0
                if self.contract_id.prestacion_id:
                    dias_aguinaldo = self.contract_id.prestacion_id.line_ids.filtered(lambda w: w.antiguedad<=antig and w.sindicalizado==self.contract_id.sindicalizado)
                    if dias_aguinaldo:
                        self.dias_aguinaldo = dias_aguinaldo[0].dias_aguinaldo
                if not dias_aguinaldo:
                    self.dias_aguinaldo = 15
    ...
    def compute_data(self):
        ...
        if dias_trabajados:
            ...
            prest_line = False
            if self.contract_id.prestacion_id:
                prest_line = self.contract_id.prestacion_id.line_ids.filtered(lambda w: w.antiguedad==_antig and w.sindicalizado==self.contract_id.sindicalizado)
            ...
            leaves = self.env['hr.leave'].search([
                                ...
                                ('holiday_status_id', '=', concepto_vacaciones.id),
                                ('date_to','<=',fecha_final),
                                ('date_from','>=',fecha_inicial if fecha_inicial.year == fecha_final.year else f_inicial)
                            ])

            dias_disfrutados_x_adelantado = int(sum(leaves.mapped('number_of_days')))

            self.proporcional_vacaciones_dias = (dias * proporcional) - dias_disfrutados_x_adelantado
        ...
        if self.company_id.crear_extra_prima_vacacional_en_aniversario=='1': # Se paga la Prima Vacacional en el aniversario, por consiguiente solo hay proporcional de Prima Vacacional
            self.proporcional_prima_vac_dias = self.proporcional_vacaciones_dias + dias_disfrutados_x_adelantado
        else:
    ...
    def create_extras_settlement(self, code=False, monto=0.0, tipo='percepcion', dias_vacaciones=0.0): # tipo: percepcion / deduccion / otrospagos
        ...
        if dias_vacaciones:
            concepto = salary_rule_obj.search([(tipo_concepto[tipo]['campo'],'=',concepto_sat.id),
                                               ('code','=','P_001_VAC'), # Fix - Agregar check a Regla Salarial
    ...
    def action_done(self):
        ...
        return {
            ...
            'name': _('Realizar Baja'),
            'res_model': 'hr.departure.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {'active_id': self.employee_id.id},
            'views': [[False, 'form']]
        }
        return True
    ...
    def action_cancel(self):
        for rec in self:
            if rec.payslip_id and rec.payslip_id.state != 'cancel':
                res = rec.payslip_id.with_context({'settlement_id' : rec.id}).action_cancel()
                _logger.info("res: %s" % res)
                if isinstance(res, dict):
                    raise ValidationError(_('Es necesario que cancele cada uno de los 2 Recibos de Nómina del Finiquito manualmente (por temas fiscales) y posteriormente cancele el Finiquito'))
            if rec.settlement_payslip_id and rec.settlement_payslip_id.state != 'cancel':
                res = rec.settlement_payslip_id.with_context({'settlement_id' : rec.id}).action_cancel()
                if isinstance(res, dict):
                    raise ValidationError(_('Es necesario que cancele cada uno de los 2 Recibos de Nómina del Finiquito manualmente (por temas fiscales) y posteriormente cancele el Finiquito'))
