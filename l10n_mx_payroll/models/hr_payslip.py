...
class HRPayslip(models.Model):
    ...
    extra_hours_qty_triple = fields.Float(string="H.E. Triples - Cant", digits=(8,2),
                                            compute=_get_data_extra_hours, store=True)
    extra_hours_dias = fields.Integer(string="H.E. Días Total", 
                                             compute=_get_data_extra_hours, store=True)
    extra_hours_dias_simple = fields.Integer(string="H.E. Simples - Días", 
                                             compute=_get_data_extra_hours, store=True)
    extra_hours_dias_doble = fields.Integer(string="H.E. Dobles - Días", 
                                            compute=_get_data_extra_hours, store=True)
    extra_hours_dias_triple = fields.Integer(string="H.E. Triples - Días ", 
                                             compute=_get_data_extra_hours, store=True) 
        ...
        if module_l10n_mx_edi:
            ...
            if not cert_str: # Validamos si el Certificado se cargo correctamente
                self.message_post(subject="Error al generar CFDI",
                                  body=_("Error en Certificado !!!\nNo puedo obtener el Certificado de Sello Digital para generar el CFDI. Revise su configuración."), subtype='notification')
                return False
            ...
            if not noCertificado: # Validamos si el Numero de Certificado se cargo correctamente
                self.message_post(subject="Error al generar CFDI",
                                  body=_("Error !!!\n\nNo se pudo obtener el Número de Certificado de Sello Digital para generar el CFDI. Por favor revise la configuración del Diario de Pago"), subtype='notification')
                return False
            ...
            if not txt_str:
                self.message_post(subject="Error al generar CFDI",
                                  body="Error en la Cadena Original !!!\nNo puedo obtener la Cadena Original del Comprobante.\n"
                                       "Revise su configuración.", subtype='notification')
                return False
    ...
    def _cron_process_payslip_2_send(self, job_count=100, days_from=15):
        ...
        for rec in payslips_to_send:
            ...
            _logger.info('Fin proceso de Envío - Recibo de Nómina CFDI: %s', fname_payslip)
            self.env.cr.commit()
    ...
    def _cron_process_payslip_web_services(self, job_count=250, days_from=15):
        ...
        for rec in payslips_to_sign:
            ...
            self.env.cr.commit()
            _logger.info('Fin proceso Timbrado - Recibo de Nómina CFDI: %s', rec.fname_payslip)
