...
class HRPayslipExtraDiscounts(models.Model):
    ...
    contract_id         = fields.Many2one('hr.contract', string='Contrato', store=False,
                                          compute="_get_contract")
    ...
    def copy(self, default=None):
        ...
        if self.company_id:
            default['name'] = self.env['ir.sequence'].with_context(force_company=self.company_id.id).next_by_code('hr.payslip.extra.discounts') or '/'
        else:
            default['name'] = self.env['ir.sequence'].next_by_code('hr.payslip.extra.discounts') or '/'
        return super(HRPayslipExtraDiscounts, self).copy(default)
...
class HRPayslip_Extra(models.Model):
    ...
    @api.depends('name', 'hr_salary_rule_id', 'employee_id')
    def _get_complete_name(self):
        for rec in self:
            rec.complete_name = "[%s] %s - [%s] %s - %s" % (rec.hr_salary_rule_id.code, rec.hr_salary_rule_id.name[:25], rec.employee_id.num_empleado, rec.employee_id.name[:25], rec.name)

    complete_name = fields.Char(string="Ref Completa", compute="_get_complete_name")
    ...
    def name_get(self):
        result = []
        for rec in self:
            name = _("[%s] %s - [%s] %s - Cant: %s - Monto: %s") % (rec.employee_id.num_empleado, rec.employee_id.name[:25], 
                                                                    rec.hr_salary_rule_id.code, rec.hr_salary_rule_id.name[:25], 
                                                                    rec.qty, rec.amount)
    ...
    def copy(self, default=None):
        default = dict(default or {})
        if self.company_id:
            default['name'] = self.env['ir.sequence'].with_context(force_company=self.company_id.id).next_by_code('hr.payslip.extra') or '/'
        else:
            default['name'] = self.env['ir.sequence'].next_by_code('hr.payslip.extra') or '/'
        return super(HRPayslip_Extra, self).copy(default)
    ...
    def action_change_state(self):
        return {
            'name': _('Cambiar estado de Extras'),
            'res_model': 'hr.payslip.extra.wizard',
            'view_mode': 'form',
            'context': {
                'active_model': 'hr.payslip.extra',
                'active_ids': self.ids,
            },
            'target': 'new',
