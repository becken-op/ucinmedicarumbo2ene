        ...
        if self.amount_select == 'percentage':
            try:
                return (float(safe_eval(self.amount_percentage_base, localdict)),
                        ...
                        float(safe_eval(self.gravado, localdict)),
                        float(safe_eval(self.exento, localdict)))
            except Exception as e:
            ...
            return float(localdict['result']), localdict.get('result_qty', 1.0), localdict.get('result_rate', 100.0), float(localdict['result_gravado']), float(localdict['result_exento'])
            #except Exception as e:
...
class HrSalaryRuleCategory(models.Model):
    ...
    def _sum_salary_rule_category(self, localdict, amount, gravado=0, exento=0):
        #_logger.info("\n\nlocaldict: %s\nself.code: %s\namount: %s\ngravado: %s\nexento: %s" % (localdict, self.code, amount, gravado, exento))
        self.ensure_one()
        if self.parent_id:
            localdict = self.parent_id._sum_salary_rule_category(localdict, amount, gravado=gravado, exento=exento)

        if not localdict['categories'].dict.get(self.code, False):
            localdict['categories'].dict[self.code] = {'amount' : 0.0, 'gravado': 0.0, 'exento' : 0.0} 
        localdict['categories'].dict[self.code]['amount']  += amount
        localdict['categories'].dict[self.code]['gravado'] += gravado
        localdict['categories'].dict[self.code]['exento']  += exento
