...
class HRContractAumentoMasivo(models.Model):
    ...
    def action_cancel(self):
        self.ensure_one()
        if self.state in ('draft','approve','confirm'):
    ...
    def action_view_lines(self):
        return {
                'name': _('Líneas de Aumento'),
                'res_model': 'hr.contract.aumento_masivo.line',
                'view_mode': 'tree,form',
    ...
    def action_compute(self):
        self.ensure_one()
        presta_obj = self.env['sat.nomina.prestaciones']
        ...
        for contract in contract_ids:
            ...
            prest_line = contract.prestacion_id.line_ids.filtered(lambda w: w.antiguedad==math.ceil(antig_laboral) and w.sindicalizado==contract.sindicalizado)
            if prest_line:
                dias_vacaciones = prest_line[0].dias_vacaciones
                dias_aguinaldo  = prest_line[0].dias_aguinaldo
                porc_prima_vacacional = prest_line[0].prima_vacacional
            else:                
                dias_aguinaldo = 15.0
            ...
            factor_integracion = round(((contract.cfdi_sueldo_base * 365.0) + aguinaldo + prima_vacacional) / (contract.cfdi_sueldo_base * 365.0), 4)
            ...
            variable = 0.0
            # Mes impar, es cuando se aplican las actualizaciones al SBC
            if self.date.month % 1 and contract.sdi_ids[0].date >= self.date.replace(day=1):
                variable  = contract.cfdi_salario_diario_integrado2 - contract.cfdi_salario_diario_preintegrado
            else: # Es mes par
                dt = datetime.combine(self.date, datetime.min.time())
                if contract.sdi_ids and contract.sdi_ids[0].date >= (dt - relativedelta(months=1)).date().replace(day=1):
                    variable  = contract.cfdi_salario_diario_integrado2 - contract.cfdi_salario_diario_preintegrado

            if not variable:
                # Variables (generalmente solo aplica Vales de Despensa)
                variable = 0.0
                if contract.prestacion_vales_despensa:
                    uma = self.env['sat.nomina.uma_umi'].search([('tipo','=','uma')], order='vigencia desc', limit=1)
                    monto_exento = uma.monto * 0.4
                    if contract.prestacion_vales_despensa > monto_exento:
                        if contract.company_id.tipo_calculo_periodo=='promedio': # 30.4
                            monto_exento_mensual = monto_exento * 30.4
                            monto_vales_despensa = round(contract.prestacion_vales_despensa * contract.company_id.tipo_calculo_periodo_dias, 0)
                            monto_gravado_mensual = monto_vales_despensa - monto_exento_mensual
                            variable = round(monto_gravado_mensual / contract.company_id.tipo_calculo_periodo_dias, 2)
                        else:
                            variable = contract.prestacion_vales_despensa - monto_exento
                # Fin Variables
    ...
    def action_confirm(self):
        ...
        for line in self.line_ids.filtered(lambda w: w.aplicar_aumento):
            _logger.info("==== %s - %s - date_start: %s - date_end: %s ====" % (line.contract_id.id, line.contract_id.name, line.contract_id.date_start, line.contract_id.date_end))
            ...
            contract = line.contract_id.copy(
                default={'cfdi_sueldo_base' : line.sal_nuevo_salario_diario,
                         'date_start'  : line.date,
                         'date_end' : contract_date_end, #line.date if line.contract_id.date_end and line.contract_id.date_end<=date_end else line.contract_id.date_end,
                         'state' : 'draft',
            ...
            if line.contract_id.date_start==line.date:
                xdate_end = line.contract_id.date_start
            else:
                xdate_end = date_end
...
class HRContractAumentoMasivoLine(models.Model):
    ...
    factor_integracion = fields.Float(string="Factor Integ.", default=0, digits=(16,4))
    ...
    parte_variable = fields.Float(
        string="Parte Variable", digits=(16,4), default=0)
    
    sal_nuevo_sbc_parte_fija = fields.Float(string="Nuevo SBC (parte fija)", digits=(16,4),)
    
    variables = fields.Float(string="Factor Integ.", default=0, digits=(16,8))
