...
class HRLeavesAllocation(models.Model):
    ...
    def get_vacations(self, xdate=(datetime.now() + timedelta(days=5)).date(),
        ...
        for contract in self.env['hr.contract'].search(dominio):
            ...
            data = {'holiday_status_id' : leave_status.id,
                    ...
                    'number_of_days'    : dias,
                    'date_from'         : xdate,
                    'date_to'           : xdate + relativedelta(years=1,days=self.env.company.dias_para_vencimiento_de_vacaciones-1),
                    'vacaciones_vigencia_inicio' : xdate,
            ...
            for line in contract.leave_allocation_ids:
                data = {
                    'holiday_status_id' : line.holiday_status_id.id,
                    'employee_id'       : contract.employee_id.id,
                    'name'              : 'Trabajador: [' +  str(contract.employee_id.id) + '] ' +\
                                            contract.employee_id.name  + ' - Contrato: ' +\
                                            contract.name + ' ' + line.holiday_status_id.name,
                    'contract_id'       : contract.id,
                    'number_of_days'    : line.qty,
                    'date_from'         : xdate,
        ...
        return {
                'name': _('Resultado Aniversario'),
                'res_model': 'hr.leave.allocation',
                'view_mode': 'tree,form',
                'context': {
                    'active_model': 'hr.leave.allocation',
                    'active_ids': leaves.ids,
                },
