...
class hr_employee_imss_movimientos_sua(models.TransientModel):
    ...
    registro_patronal_ids = fields.Many2many('hr.registro_patronal', 'hr_registro_patronal_sua_rel',
        'wizard_id','registro_patronal_id',
        string="Registros Patronales")
    ...
    def create_files(self, lines):
        ...
        if self.type=='incap':            
            idse_filename = periodo + 'INCAPACIDADES' + '.txt'
            sua_asegurados = base64.encodebytes(self.write_file('\r\n'.join([l.line_sua_datos for l in lines])))
            sua_asegurados_filename = periodo + '_SUA_ASEGURADOS.txt'
            sua_movimientos = base64.encodebytes(self.write_file('\r\n'.join([l.line_sua_movimientos for l in lines])))
            ...
            sua_afiliacion = base64.encodebytes(self.write_file('\r\n'.join([l.line_sua_afiliacion for l in lines])))
    ...
    def get_sua_files(self):        
        ...
        if self.type=='incap':
            ...
            incapacity_line_obj = self.env['hr.employee.imss.incapacity.line']
            ...
            if self.registro_patronal_ids:
                domain.append(('contract_id.registro_patronal_id','in',self.registro_patronal_ids.ids))
                
        
            lines = incapacity_line_obj.search(domain)
            if not lines:
