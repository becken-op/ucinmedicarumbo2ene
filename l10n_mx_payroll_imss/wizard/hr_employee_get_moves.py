    ...
    def create_records(self):
        ...
        if self.type=='08': # Altas o Reingresos
            ...
            settlements = self.env['hr.settlement'].search([('state','=','done'),
                                                            ('date','>=',self.date_from),
                                                            ('date','<=',self.date_to),
                                                            '|', ('employee_imss_id','=',False),
                                                            ('employee_imss_id.state','=','cancel'),
                                                            ])
            if settlements:
                sett_data = []
                for sett in settlements:
                    sett_data.append({
                        ...
                        'causa_baja'  : sett.causa_baja,
                        })
                #_logger.info("sett_data: %s" % sett_data)
        ...
        if self.type=='02': #Bajas
            lines = []
            _logger.info("sett_data: %s" % sett_data)
            for sett in sett_data:
                lines.append((0,0,
                          {'employee_id' : sett['contract_id'].employee_id.id,
                           'cfdi_sueldo_base' : sett['contract_id'].cfdi_salario_diario_integrado2 or contract.cfdi_salario_diario_integrado,
                           'contract_id' : sett['contract_id'].id,
                           'causa_baja' : sett['causa_baja'],
                           'settlement_id' : sett['settlement_id'].id,
                          }))
            _logger.info("lines: %s" % lines)
            mov_data['line_ids'] = lines
        
        mov_id = employee_imss_obj.create(mov_data)
        
        if self.type=='02': #Bajas
            for sett in sett_data:
                sett['settlement_id'].employee_imss_id = mov_id.id
