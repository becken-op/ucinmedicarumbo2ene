...
class HREmployeeIMSSSSBC(models.Model):
    ...
    imss_id = fields.Many2one('hr.employee.imss', string="Modificación IMSS", index=True)
    _sql_constraints = [
    ...
    def action_compute(self):
        ...
        for contract in contract_ids:
            ...
            prestacion = False
            if contract.prestacion_id:
                prestacion = contract.prestacion_id.line_ids.filtered(lambda w: w.antiguedad==math.ceil(antig_laboral) and w.sindicalizado==contract.sindicalizado)
            if prestacion:            
            ...
            if not sbc_nuevo_parte_variable:
                continue
    ...
    def action_confirm(self):
        ...
        for l in self.line_ids.filtered(lambda w: w.aplicar_modificacion_sbc):
            ...
            res = hr_contract_sdi_obj.create({
                        ...
                        'notes'       : _('Actualización de SBC Bimestral'),
    ...
    def action_view_details(self):
        action = self.env["ir.actions.actions"].sudo()._for_xml_id('l10n_mx_payroll.action_hr_payslip_analysis')
        action.update({
            'name'      : _('Análisis IMSS %s') % self.name,
            'domain'    : "[('imss_gravado','!=',0), ('state','=','done'),('date_from','>=','%s'),('date_to','<=','%s'), ('employee_id','in',[%s])]" % \
                            (self.date_from.isoformat(),
                             self.date_to.isoformat(),
                             ','.join([str(x.employee_id.id) for x in self.line_ids])
                            ),
            'context'   : "{'pivot_measures': ['imss_gravado'], 'pivot_row_groupby': ['employee_id','salary_rule_id'],'pivot_column_groupby': ['date_from:day'],}",
            })
        return action
...
class HREmployeeIMSSSSBCLine(models.Model):
    ...
    def _get_line(self):
        for l in self:
            ...
            if not l.employee_id.contract_id.registro_patronal_id.name or len(l.employee_id.contract_id.registro_patronal_id.name) != 11:
                raise ValidationError(_('Error !\nEl Trabajador no tiene asociado Registro Patronal o se encuentra mal definido, por favor revise'))
    ...
    registro_patronal_id = fields.Many2one(related="contract_id.registro_patronal_id")
    ...
    registro_patronal = fields.Char(related="registro_patronal_id.name", readonly=True)
    ...
    def action_view_details(self):
        ...
        action.update({
            ...
            'domain'    : "[('imss_gravado','!=',0), ('employee_id','=',%s),('state','=','done'),('date_from','>=','%s'),('date_to','<=','%s')]" % \
                            (self.employee_id.id, 
                             desde.isoformat(),
                             self.imss_sbc_id.date_to.isoformat()
                            ),
            'context'   : "{'pivot_measures': ['imss_gravado'], 'pivot_row_groupby': ['salary_rule_id'],'pivot_column_groupby': ['date_from:day'],}"
            })
        return action
