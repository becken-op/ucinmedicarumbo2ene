...
class HREmployeeIMSS(models.Model):
    ...
    imss_sbc_id = fields.Many2one('hr.employee.imss.sbc', 
        string="Actualización Bimestral SBC", index=True)
    ...
    def create_files(self):
        ...
        for rec in self:
            idse = base64.encodebytes(rec.write_file('\n'.join([l.line_idse for l in rec.line_ids])))
            idse_filename = rec.date.isoformat() + ('_ALTAS_REINGRESOS_IDSE' if rec.type=='08' else '_MODIFICACIONES_IDSE' if rec.type=='07' else '_BAJAS_IDSE' if rec.type=='02' else '_NO_DEFINIDO') + '.txt'
            
            sua_afiliacion = base64.encodebytes(rec.write_file('\n'.join([l.line_sua_afiliacion for l in rec.line_ids])))
            sua_afiliacion_filename = rec.date.isoformat() + '_SUA_AFILIACION.txt'
            
            sua_asegurados = base64.encodebytes(rec.write_file('\n'.join([l.line_sua_asegurado for l in rec.line_ids])))
            sua_asegurados_filename = rec.date.isoformat() + '_SUA_ASEGURADOS.txt'
            
            sua_movimientos = base64.encodebytes(rec.write_file('\n'.join([l.line_sua_movimiento for l in rec.line_ids])))
            sua_movimientos_filename = rec.date.isoformat() + '_SUA_MOVIMIENTOS.txt'
    ...
    def action_cancel(self):
        if self.imss_sbc_id and self.imss_sbc_id.state != 'cancel':
            raise ValidationError(_("No puede cancelar el registro porque fue generado desde una Actualización Bimestral de SBC. Primero cancele ese registro."))

        settlement_obj = self.env['hr.settlement']
        for rec in self.filtered(lambda w: w.type=='02'):
            settlements = settlement_obj.browse(rec.line_ids.mapped('settlement_id.id'))
            settlements.write({'employee_imss_id' : False})
...
class HREmployeeIMSSLine(models.Model):
    ...
    def get_idse_line(self):
        if self.type in ('08','07'): # Alta o Reingreso / Modificacion
            return '%s%s%s%s%s%s      %s%s%s%s%s%s%s%s %s9' % \
                    (self.employee_id.contract_id.registro_patronal_id.name,
                     self.employee_id.nss,
                     self.employee_id.apellido_paterno.replace('ñ','n').replace('Ñ','N').ljust(27, ' ')[:27],
                     self.employee_id.apellido_materno and self.employee_id.apellido_materno.replace('ñ','n').replace('Ñ','N').ljust(27, ' ')[:27] if self.employee_id.apellido_materno else (' '*27),
                     self.employee_id.nombre.replace('ñ','n').replace('Ñ','N').ljust(27, ' ')[:27],
                     '{:07.2f}'.format(self.employee_id.contract_id.cfdi_salario_diario_integrado2 or self.employee_id.contract_id.cfdi_salario_diario_integrado).replace('.',''),
                     
                     self.employee_id.contract_id.tipo_trabajador,
                     self.employee_id.contract_id.tipo_salario,
                     self.employee_id.contract_id.jornada_reducida,
                     (self.employee_id.contract_id.date_start if self.type=='07' else self.employee_id.contract_id.fecha_ingreso).strftime('%d%m%Y'),
            ...
            return '%s%s%s%s%s000000000000000%s     %s%s%s%s%s9' % \
                    (self.employee_id.contract_id.registro_patronal_id.name,
                     self.employee_id.nss,
                     self.employee_id.apellido_paterno.replace('ñ','n').replace('Ñ','N').ljust(27, ' ')[:27],
                     self.employee_id.apellido_materno and self.employee_id.apellido_materno.replace('ñ','n').replace('Ñ','N').ljust(27, ' ')[:27] or '', 
                     self.employee_id.nombre.replace('ñ','n').replace('Ñ','N').ljust(27, ' ')[:27],
                     self.employee_id.contract_id.date_end.strftime('%d%m%Y'),
                     self.type,
                     '{:05}'.format(self.employee_id.clave_subdelegacion),
                     self.employee_id.num_empleado.ljust(10, ' ')[:10],
                     self.causa_baja,
    ...
    def get_sua_line(self):
        ...
        if self.employee_id.state_of_birth.country_id.code=='MX' and not self.employee_id.state_of_birth:
            raise ValidationError(_('Error !\nEl Estado de nacimiento del Trabajador %s no está capturado)') % self.employee_id.name)    
        if self.employee_id.gender not in ('male','female'):
            raise ValidationError(_('Error !\nEl género del Trabajador %s no está capturado correctamente') % self.employee_id.name)
        

        #self.employee_id.place_of_birth.ljust(25, ' ')[:25],
        #self.employee_id.state_of_birth.country_id.code=='MX' and self.employee_id.state_of_birth.imss_code or '00',
        #self.employee_id.contract_id.job_id and self.employee_id.contract_id.job_id.name.ljust(12, ' ')[:12] or (' '*12),
        l_sua_afiliacion = '%s%s%s%s%s%s%s%s%s%s%s' % \
                (self.employee_id.contract_id.registro_patronal_id.name,
                 self.employee_id.nss,
                 self.employee_id.address_home_id.zip or self.employee_id.zip,
                 self.employee_id.birthday.strftime('%d%m%Y'),
                 self.employee_id.state_of_birth.country_id.code=='MX' and self.employee_id.state_of_birth.name.replace('á','a').replace('é','e').replace('í','i').replace('ó','o').replace('ú','u').ljust(25, ' ')[:25] or (' ' * 25), # Reemplazar acentos por vocales regulares
                 '00',         
        ...
        #_logger.info("tipo_salario: %s - %s" % (self.employee_id.name, self.employee_id.contract_id.tipo_salario))
        if self.type=='08': # Alta o reingreso
            x_date = self.employee_id.contract_id.fecha_ingreso
            _logger.info("self.employee_id.contract_id.fecha_ingreso: %s" % self.employee_id.contract_id.fecha_ingreso)
        elif self.type=='07': # Modificacion
            if self.imss_id.imss_sbc_id:
                x_date = self.imss_id.imss_sbc_id.date
            else:
                x_date = self.employee_id.contract_id.date_start
            _logger.info("self.employee_id.contract_id.date_start: %s" % self.employee_id.contract_id.date_start)
        elif self.type=='02': # Modificacion
            x_date = self.employee_id.contract_id.date_end
            _logger.info("self.employee_id.contract_id.date_end: %s" % self.employee_id.contract_id.date_end)
        _logger.info("self.type: %s" % self.type)
        _logger.info("x_date: %s" % x_date)
        ...
        if self.employee_id.infonavit_ids:
            ...
            if self.employee_id.infonavit_ids[0].tipo == 'porcentaje':
                factor_infonavit = '{:09.2f}'.format(factor).replace('.','')
            elif self.employee_id.infonavit_ids[0].tipo == 'importe':
                factor_infonavit = '{:09.3f}'.format(factor/2).replace('.','')
            elif self.employee_id.infonavit_ids[0].tipo == 'veces_umi':
                factor_infonavit = '{:09.4f}'.format(factor).replace('.','')
            

        l_sua_asegurado = '%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s' % \
                (self.employee_id.contract_id.registro_patronal_id.name,
                 self.employee_id.nss,
                 self.employee_id.vat or self.employee_id.address_home_id.vat,
                 ...
                 self.employee_id.contract_id.tipo_trabajador, # Tipo de Trabajador
                 self.employee_id.contract_id.jornada_reducida, # Jornada Reducida
                 self.employee_id.contract_id.fecha_ingreso.strftime('%d%m%Y'), # Fecha Alta
                 '{:08.2f}'.format(self.contract_id.cfdi_salario_diario_integrado2 or self.contract_id.cfdi_salario_diario_integrado).replace('.',''),
                 self.employee_id.num_empleado.ljust(5, ' ')[:5],
                 ' '*12,
                 self.employee_id.infonavit_ids and self.employee_id.infonavit_ids[0].name.ljust(10, ' ')[:10] or '                  000000000',
                 self.employee_id.infonavit_ids and self.employee_id.infonavit_ids[0].vigencia.strftime('%d%m%Y') or '',
    ...
    def _get_line(self):
        for l in self:
            # Validaciones IDSE
            if not l.employee_id.contract_id.registro_patronal_id.name or len(l.employee_id.contract_id.registro_patronal_id.name) != 11:
                raise ValidationError(_('Error !\nEl Trabajador no tiene asignado Registro patronal  o se encuentra mal definido, por favor revise'))
            ...
            if l.type in ('07','08'):
                if not l.employee_id.contract_id.tipo_salario:
                    raise ValidationError(_('Error !\nNo está definido el valor del Tipo de Salario para %s') % l.employee_id.name)
                if not l.employee_id.contract_id.jornada_reducida:
    ...
    settlement_id = fields.Many2one('hr.settlement', string="Finiquito")
...
class HRContractAumentoMasivo(models.Model):
    ...
    imss_modif_id = fields.Many2one('hr.employee.imss', string="Modificación IMSS", readonly=True)
    ...
    def create_imss_record(self):
        ...
        data ={'date'       : self.date,
               ...
               'notes'      : _('Registro creado desde Aumento Masivo %s' % self.name)
              }
