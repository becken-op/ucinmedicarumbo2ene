...
class hr_settlement(models.Model):
    ...
    employee_imss_id = fields.Many2one('hr.employee.imss', string="Registro de Baja")
    

    def action_done(self):
        res = super(hr_settlement, self).action_done()
        wizard_obj = self.env['hr.settlement.baja_imss.wizard']
        imss_obj = self.env['hr.employee.imss']
        for rec in self:
            baja_wiz = wizard_obj.create({
                'date' : rec.date,
                'hr_settlement_ids' : [(6,0,[rec.id])],
                })
            baja = imss_obj.browse(baja_wiz.crear_baja_imss()['res_id'])
            baja.action_confirm()
            rec.employee_imss_id = baja.id
        return res


    def action_cancel(self):
        res = super(hr_settlement, self).action_cancel()
        for rec in self:
            rec.employee_imss_id.action_cancel()
        return res
...
class hr_settlement_crea_baja_imss(models.TransientModel):
    ...
    def default_get(self, default_fields):
        ...
        res.update({'hr_settlement_ids'    : [(6,0, record_ids)],
    ...
    def crear_baja_imss(self):
        ...
        self.hr_settlement_ids.write({'employee_imss_id' : baja_id.id})
