...
class HREmployeeIMSSIncapacityLine(models.Model):
    ...
    def get_idse_line(self):
        return '%s%s' % \
                (self.leave_id.employee_id.contract_id.registro_patronal_id.name,
                 self.employee_id.nss
                )
    
    def get_sua_line(self):

        # ${Registro Patronal(11)}
        # ${NSS(11)}
        # ${Tipo Incanacidad(1)}
        # ${Control Incapacidad(1)}
        # ${Fecha inicial (6 - DDMMYYYY)}
        # ${Folio Incapacidad (8)}
        # ${Dias Incapacidad (2)}
        l_sua_movimientos = '%s%s%s%s%s%s%s' % \
                (self.leave_id.employee_id.contract_id.registro_patronal_id.name,
                 self.employee_id.nss,
                 '12',
                 #str(int(self.leave_id.tipoincapacidad_id.code)), # Tipo de Incapacidad
                 #self.leave_id.incapacidad_control,  # Control Incapacidad
                 self.date_from.strftime('%d%m%Y'),
                 self.leave_id.incapacidad_folio.ljust(8, ' ')[:8], # 8 posiciones
                 '{:02}'.format(int(self.leave_id.incapacidad_dias)), # Num dias "subsidiados"
                 '0000000'
                )
        ...
        l_sua_datos = '%s%s%s%s%s%s%s%s%s%s%s%s' % \
                (self.leave_id.employee_id.contract_id.registro_patronal_id.name,
                 ...
                 '{:03}'.format(int(self.leave_id.incapacidad_dias)), # Num dias "subsidiados"
    ...
    def _get_line(self):
        for l in self:
            # Validaciones IDSE
            if not l.leave_id.employee_id.contract_id.registro_patronal_id.name or len(l.leave_id.employee_id.contract_id.registro_patronal_id.name) != 11:
                raise ValidationError(_('Error !\nEl Trabajador no tiene asociado Registro Patronal en su contrato o el Registro Patronal se encuentra mal definido, por favor revise'))
    ...
    state = fields.Selection(related="incapacity_id.state", store=True, index=True)
    leave_id = fields.Many2one('hr.leave', string="Ausencia", required=True,
                               index=True, readonly=True)
    
    number_of_days = fields.Integer(string="# Días", related="leave_id.incapacidad_dias",
