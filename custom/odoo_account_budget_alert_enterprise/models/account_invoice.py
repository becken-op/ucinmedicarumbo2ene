...
class AccountMove(models.Model):
    ...
    def check_budge_account(self):
        for rec in self:
            custom_date = rec.invoice_date or fields.date.today()
            ...
            for invoice_line in rec.invoice_line_ids:
                for account_budget_line in account_budget_line_ids:
                    budget_line_ids  = account_budget_line.filtered(
                        lambda m: m.analytic_account_id == invoice_line.analytic_account_id
                        and invoice_line.account_id in m.general_budget_id.account_ids)
                    if budget_line_ids:
                        total_amount = sum(line.practical_amount for line in budget_line_ids)
                        budget_currency_price = rec.currency_id._convert(
                        ...
                        if rec.move_type in ('in_invoice', 'in_refund'):
                            budget_currency_price *= -1
                        total_practical_amount = total_amount + budget_currency_price
                        
                        if account_budget_line.custom_configuration_state in ['warn', 'warn_stop']:
                            _logger.info("total_practical_amount: %s, total_amount: %s" % (total_practical_amount, total_amount))
                            ...
                            if budget_current_percentaje > self.company_id.budget_warning_percentage:
                                ...
                                warn_msg += _('* Account "%s", Analytic Account "%s" exceeds the %s%% warning percentage, budget: %s (%s vs %s wich represents %s%%).\n') % (invoice_line.account_id.name, account_budget_line.analytic_account_id.name, self.company_id.budget_warning_percentage, account_budget_line.crossovered_budget_id.name, planned_amount_str, practical_amount_str, budget_current_percentaje)
                        if abs(account_budget_line.planned_amount) < abs(total_practical_amount):
                            if account_budget_line.custom_configuration_state in ['stop', 'warn_stop']:
                                ...
                                practical_amount_str = formatLang(self.env, abs(total_practical_amount), currency_obj=account_budget_line.currency_id)
                                planned_amount_str = formatLang(self.env, abs(account_budget_line.planned_amount), currency_obj=account_budget_line.currency_id)
                                stop_msg += _('* Account "%s", Analytic Account "%s" exceeds budget: %s (%s vs %s).\n') % (invoice_line.account_id.name, account_budget_line.analytic_account_id.name, account_budget_line.crossovered_budget_id.name, planned_amount_str, practical_amount_str)
    ...
    def action_post(self):
        if self._context.get('active_id',False):
            move_browse_id = self.env['account.move'].browse(self._context.get('active_id',False))
            if not move_browse_id.move_type in ('out_invoice','in_invoice'):
                return super(AccountMove, self).action_post()
                
        if not self._context.get('default_move_type',False) in ('out_invoice','in_invoice'):
            return super(AccountMove, self).action_post()
        account_budget_line_ids = self.check_budge_account()
    ...
    @api.depends('invoice_line_ids', 'warn_msg')
    def generate_warn_msg(self):
        for rec in self:
            ...
            rec.warn_msg = warn_msg
            
            if rec.warn_msg == '':
                rec.write({'custom_is_confirm': False})
            else:
                rec.write({'custom_is_confirm': True})
