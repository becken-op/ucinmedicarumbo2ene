...
class Inventory(models.Model):
    ...
    def action_validate(self):
        ...
        if not self.user_has_groups('stock_inventory_cost_info_validation.group_unrestrict_products_manager'):
            if not self.user_has_groups('stock.group_stock_user'):
                raise UserError(_("Only a stock user can validate an inventory adjustment."))

            stock_inventory_user_validation_max_amount = self.company_id.stock_inventory_user_validation_max_amount
            # Compute de total cost of the adjusment
            total_adjustment_cost = 0.00
            for line_id in self.line_ids:
                total_adjustment_cost += line_id.adjustment_cost
            if stock_inventory_user_validation_max_amount < abs(total_adjustment_cost):
                raise UserError(_("Only a inventory adjusment manager can validate this inventory adjustment, because the adjustment cost total is greater than %.2f %s.") % (stock_inventory_user_validation_max_amount, self.company_id.currency_id.name))
