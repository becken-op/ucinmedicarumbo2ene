...
class SaleOrderLine(models.Model):
    ...
    def get_sale_order_line_multiline_description_sale(self, product):
        ...
        partner_id = self.order_id.partner_id.commercial_partner_id
        if product.customer_ids:
            customer_id = self.env['product.customer.info'].search(
                [('product_tmpl_id', '=', product.product_tmpl_id.id), ('partner_id', '=', partner_id.id)], limit=1)
            if customer_id.partner_id.commercial_partner_id.id == partner_id.id:
                if customer_id.customer_product_name and customer_id.customer_product_code:
                    ...
                    new_desc = '[' + new_code + '] ' + new_name + ' (' + product.default_code + ')'
